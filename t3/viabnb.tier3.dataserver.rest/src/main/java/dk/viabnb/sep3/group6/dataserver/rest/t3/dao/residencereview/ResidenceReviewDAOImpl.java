...
public class ResidenceReviewDAOImpl extends BaseDao
    implements ResidenceReviewDAO
{
  @Override public ResidenceReview createResidenceReview(Residence residence,
      ResidenceReview residenceReview)
  {
    try (Connection connection = getConnection())
    {
      PreparedStatement stm = connection.prepareStatement(
          "INSERT INTO residencereview(residencerating, residencereviewtext, residenceid, guestid, createddate) values (?,?,?,?,?)");
  ...
  @Override public ResidenceReview updateResidenceReview(int residenceId,
  ...
  @Override public List<ResidenceReview> getAllResidenceReviewsByResidenceId(
      int residenceId)
  {
    ...
    try (Connection connection = getConnection())
    {
      ...
      while (result.next())
      {
        ResidenceReview residenceReview = new ResidenceReview(
            ...
            result.getString("residencereviewtext"), result.getInt("viaid"),
  ...
  private int getGuestIdByViaId(int viaId)
  {
    ...
    {
      PreparedStatement stm = connection.prepareStatement(
          "select guestid from guest where viaid = ?");
      stm.setInt(1, viaId);
      ResultSet result = stm.executeQuery();
