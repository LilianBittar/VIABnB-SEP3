...
namespace UnitTests.RentalServiceTests
{
    public class GetRentRequestTest
    {
        ...
        [Test]
        public void GetAllNotAnsweredRentRequestDoesNotThrowExceptionsTest()
        {
            ...
            var requestList = new List<RentRequest>();
            requestList.Add(request);
            _rentRequestRepository.Setup(exp => exp.GetAllNotAnsweredRentRequestAsync().Result)
                .Returns(requestList);
            Assert.DoesNotThrowAsync(async () => await _rentalService.GetAllNotAnsweredRentRequestAsync());
        }
